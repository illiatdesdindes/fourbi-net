...
module ActiveRecord
  module ConnectionAdapters
    class Column
      ...
      def get_value_for_index column_name
        case column_name
          when 'id' then '_'
          when 'created_at' then 'zzz'
          when 'updated_at' then 'zzzzz'
          else column_name
        end
      end

      def <=> o
        get_value_for_index(name) <=> get_value_for_index(o.name)
      end
    end
  end
end
...
module AnnotateModels
  ...
  def self.get_schema_info(klass, header)
    ...
    klass.columns.sort.each do |col|
      attrs = []
